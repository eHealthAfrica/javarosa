...
public class FormDefCacheInternal2ndryInstanceBenchmark {
    ...
    @Benchmark
    public void
    benchmark1FormDefCacheWriteToCacheBefore(FormDefCacheState state, Blackhole bh) throws IOException {
        FormDefCache.writeCache(state.formDef, state.resourcePath.toString(), state.cachePath);
    }

    @Benchmark
    public void
    benchmark2FormDefCacheReadFromCacheBefore(FormDefCacheState state, Blackhole bh) throws IOException {
    ...
    @Benchmark
    public void
    benchmark1FormDefCacheWriteToCacheAfter(FormDefCacheState state, Blackhole bh) throws IOException {
        FormDefCache.writeCache(state.formDef, state.resourcePath.toString(), state.cachePath);
    }

    @Benchmark
    public void
    benchmark2FormDefCacheReadFromCacheAfter(FormDefCacheState state, Blackhole bh) throws IOException {
        FormDef cachedFormDef = FormDefCache.readCache(state.resourcePath.toFile(), state.cachePath);
        bh.consume(cachedFormDef);
    }

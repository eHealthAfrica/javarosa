...
public class XFormBuilder {
    ...
    private XFormBuilder buildExternalSecondaryInstances() {
        Map<String, Path> paths = createExternalInstances(dummyXForm.getExternalOptionSelectorList());
    ...
    private Map<String, Path> createExternalInstances(List<OptionSelector> optionSelectorList) {
        final String ROOT = dummyXForm.getMainInstanceTagName();
        ...
        try {
            for (OptionSelector optionSelector : optionSelectorList) {
                ...
                fileWriter.close();
                fileLocationMap.put(optionSelector.getInstanceId(), externalInstanceFile.toPath());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
